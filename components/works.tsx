...
const Works = () => {
  ...
  return (
    <motion.section id="works" className="mt-4" initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2, duration: 0.3 }}>
          ...
          currentProjects.map(project => {
            return (
              <li key={project.title} className="mt-2">
                ...
                { project.link && <a className="text-gray-900 dark:text-gray-50 hover:text-orange-500 dark:hover:text-orange-400 hover:underline hover:underline-offset-2" href={project.link}>{project.link}</a> }
